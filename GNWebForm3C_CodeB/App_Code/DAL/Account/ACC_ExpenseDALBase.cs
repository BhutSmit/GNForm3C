...
namespace GNForm3C.DAL
{
	public abstract class ACC_ExpenseDALBase : DataBaseConfig
	{
				...
				using (IDataReader dr = DBH.ExecuteReader(sqlDB, dbCMD))
				{
					while (dr.Read())
					{
                        ...
                        if (!dr["TagName"].Equals(System.DBNull.Value))
                            entACC_Expense.TagName = dr["TagName"].ToString();
        ...
        public DataTable SelectByHospital(SqlInt32 HospitalID)
        {
            ...
            {
                SqlDatabase sqlDB = new SqlDatabase(myConnectionString);
                DbCommand dbCMD = sqlDB.GetStoredProcCommand("PR_HospitalWise_ACC_Expense");
				sqlDB.AddInParameter(dbCMD, "@HospitalID", SqlDbType.Int, HospitalID);

                DataTable dtACC_Expense = new DataTable("PR_HospitalWise_ACC_Expense");
